...
async function generatePayableNote(callback) {
  const doc = new jsPDF();
  ...
  async function createHeader(items) {
    ...
    doc.setFontSize(26);
    doc.text("เอกสารตั้งหนี้",200, 25,'right');
    doc.setFontSize(16);
    doc.text("Payable Note",200, 30,'right');
  ...
  async function createFooter(items) {
    ...
    doc.setFontSize(12);
    doc.setTextColor(0,0,0);
    doc.text("ยอมรับเอกสารตั้งหนี้ / Accepted by", 154.5, 252);
    doc.setLineWidth(0.1);
    doc.line(160, 270, 192.5, 270); //ลายเซ็น
    // await addImageToPDF(items.signature, 161.35, 255, 30.8, 14.23);
    doc.setFont('THSarabunNew','normal');
